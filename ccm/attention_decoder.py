...
def attention_decoder_fn_train(encoder_state,
    ...
    Args:
        encoder_state: 编码器最后一层最后一个 cell 的输出，用来在 decoder 第一个时间步初始化 cell_state 和 attention
        attention_keys: 用于与 query(上个时间步的输出) 进行比较
        attention_values: 用于 soft attention 计算注意力权重
        attention_score_fn: 返回上下文的函数
        attention_construct_fn: 返回拼接好的上下文的函数
        name: (default: `None`) decoder 函数的 namescope，默认为 'simple_decoder_fn_train'
    """
    ...
    def decoder_fn(time, cell_state, cell_input, cell_output, context_state):
        ...
        Args:
            time: 记录时间步
            cell_state: RNNCell 的长时记忆，在第一个时间步用 encoder 的最后输出去初始化，之后都不用去管
            cell_input: 下一步的输入
            cell_output: RNNCell 上一步的输出
            context_state: 用于记录注意力系数 alignments 的 TensorArray
        """
...
def attention_decoder_fn_inference(output_fn,  #
    ...
    Args:
        output_fn: 一个输出函数，用于将你的 cell_output 映射到分类的预测。如果传入的是 None，那么它将作为一个恒等函数（identity function），它在使用 RNNCell `OutputProjectionWrapper` 可能会用到。
        encoder_state: 初始化 dynamic_rnn_decoder 的编码的状态
        attention_keys: 与 query 进行比较
        attention_values: 用于构建上下文
        attention_score_fn: 得到上下文的函数
        attention_construct_fn: 得到 attention 的函数
        embeddings: 词汇表的嵌入矩阵，size: [num_decoder_symbols, embedding_size]
        start_of_sequence_id: GO_ID 2
        end_of_sequence_id: EOS_ID 3
        maximum_length: 解码最大允许的时间步
        num_decoder_symbols: 每个时间步能够解码的分类数量，即词汇表大小
        dtype: (default: `dtypes.int32`) 当处理整数对象时默认的数据类型
        name: (default: `None`) decoder函数的 namescope
            默认为 simple_decoder_fn_train
    """
    ...
    def decoder_fn(time, cell_state, cell_input, cell_output, context_state):
        ...
        这个 decoder 函数和 attention_decoder_fn_train 中的 decoder_fn 的主要区别是
        next_cell_input 是如何计算的。在 decoder 中，我们通过在解码器输出的特征维度上
        运用一个 argmax 来计算下个输入。这是一种 greedy-search 的方式。(Bahdanau et al., 2014)
        & (Sutskever et al., 2014) 却使用 beam-search .

        Args:
            time: 时间步
            cell_state: RNNCell 状态
            cell_input: dynamic_rnn_decoder 提供的输入
            cell_output: RNNCell 的输出
            context_state: dynamic_rnn_decoder 提供的上下文状态

        返回:
            一个元祖 (done, next state, next input, emit output, next context state)
            其中:
            done: 一个布尔向量，用来指示哪个句子已经达到了 end_of_sequence_id。这被 dynamic_rnn_decoder 用来提早停止。
                当 time>=maximum_length 时，一个所有元素均为 true 的布尔向量被返回。
            next state: 这个 decoder 函数不修改给定的状态
            next input: 来自于`cell_output` 的 argmax 的嵌入被用于 next_input
            emit output: 如果 output_fn is None，所提供的 cell_output 被返回。
                否则，output_fn 在计算 next_input 和返回 cell_output 之前被用于更新 cell_output
            next context state: `context_state`, 这个 decoder 函数不修改给定的上下文状态。
...
def prepare_attention(attention_states,  # 编码器输出: 每个 batch，每个时间步的输出
    ...
    返回:
        attention_keys: 用来和 query 比较得到注意力系数的 keys
        attention_values: 用来乘以注意力相关系数的 value
        attention_score_fn: 用来计算上下文的函数
        attention_construct_fn: 用来计算拼接后上下文的函数
    Args:
        attention_states: 编码器输出: 每个 batch，每个时间步的输出
        attention_option: 如何计算注意力，luong 或 bahdanau
        num_units: 隐藏状态维度
        reuse: 是否重复使用变量域
    """
...
'''
# reduce_sum(v * math_ops.tanh(keys + query), [2])
...
def _create_attention_score_fn(name,
    ...
    Args:
        ...
        attention_option: 如何计算注意力，luong 或 bahdanau
        reuse: 是否重复使用变量域
        dtype: (default: `dtypes.float32`) 要使用的数据类型
    """
    with variable_scope.variable_scope(name, reuse=reuse):
        # 如果用的是 bahdanau，就先声明两个变量
        ...
        def attention_score_fn(query, keys, values):
            ...
            返回:
                context_vector: [batch_size, num_units] 可以是上下文，也可以是图的上下文
                alignments: 计算上下文时:[batch_size, attention_length],计算图上下文时: [batch_size, triple_num] 上下文或者图上下文的注意力系数
                final_alignments: [batch_size, triple_num, triple_len] 对图的注意力系数 * 对三元组的注意力系数

            Args:
                query: [batch_size, num_units] 一般是上个时间步的输出
                keys: 不是元组时: [batch_size, attention_length, num_unit]
                    是元组时: (graph_keys, triples_keys)
                               graph_keys: [batch_size, triple_num, num_unit]
                               triples_keys: [encoder_batch_size, triple_num, triple_len, num_unit]
                values: 不是元组时: [batch_size, attention_length, num_units]
                    是元组时: 类似 keys
            """
            ...
            # 当 keys 为元组时
            # keys 为知识图向量 [batch_size, triple_num, num_units]
            # triple_keys 为三元组嵌入数组 [batch_size, triple_num, triple_len, num_units]
            # value 的维度同 keys
            if type(keys) is tuple:
                keys, triple_keys = keys
                values, triple_values = values

            # scores = reduce_sum(v * math_ops.tanh(keys + query), [2]) 为没有 softmax 的注意力系数
            if attention_option == "bahdanau":
                # 转换 query，将 query 乘以参数矩阵 query_w
                query = math_ops.matmul(query, query_w)  # [batch_size, num_units]
                query = array_ops.reshape(query, [-1, 1, num_units])  # [batch_size, 1, num_units]
                # attn_fun: reduce_sum(v * math_ops.tanh(keys + query), [2])
                scores = _attn_add_fun(score_v, keys, query)  # [batch_size, attention_length]

            # 实验中用来计算图上下文，也可以选来计算上下文
            # scores = reduce_sum(keys * query, [2]) 为没有 softmax 的注意力系数
            elif attention_option == "luong":
                query = array_ops.reshape(query, [-1, 1, num_units])  # [batch_size, 1, num_units]
                scores = _attn_mul_fun(keys, query)  # [batch_size, triple_num]
            else:
                raise ValueError("Unknown attention option %s!" % attention_option)

            # 计算 alignment 权值
            # scores: 计算上下文时:[batch_size, attention_length],计算图上下文时: [batch_size, triple_num]
            # alignments: 计算上下文时:[batch_size, attention_length],计算图上下文时: [batch_size, triple_num]
            # TODO(thangluong): not normalize over padding positions.
            alignments = nn_ops.softmax(scores)

            # 计算带注意力的上下文向量，或计算带注意力的三元组向量
            # new_alignments: 计算上下文时:[batch_size, attention_length, 1],计算图上下文时: [batch_size, triple_num, 1]
